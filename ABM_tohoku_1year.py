...
PREF_MAP_NPY = r"G:\マイドライブ\クマ出没予測\試作\ABM_2\環境データ\pref_code.npy"
HABITAT_SUITABILITY_NPY = r"G:\マイドライブ\クマ出没予測\試作\ABM_2\環境データ\initial_habitat_suitability.npy"

WEATHER_CSVS = {
        1: r"G:\マイドライブ\クマ出没予測\試作\ABM_2\wether_aomori_2025.csv",
        2: r"G:\マイドライブ\クマ出没予測\試作\ABM_2\wether_iwate_2025.csv",
        3: r"G:\マイドライブ\クマ出没予測\試作\ABM_2\wether_miyagi_2025.csv",
        4: r"G:\マイドライブ\クマ出没予測\試作\ABM_2\wether_akita_2025.csv",
        5: r"G:\マイドライブ\クマ出没予測\試作\ABM_2\wether_yamagata_2025.csv",
    }
...
X_SIZE, Y_SIZE = 2687, 5210  # env[:, y, x]
NUM_ENSEMBLE = 1
...
TARGET_DATA = {
    ...
    "poor_mast": {
        ...
        # 4. 【秋田県】年間捕殺割合
        # 例：7500頭に対し、年間2000頭捕殺されたなら 2000/7500 = 0.267 (26.7%)
        "annual_kill_rate_akita": 0.267,
...
class SimConfig:
    ...
    """
    # 飢餓時の死亡確率を決める係数 k
    starvation_mortality_k: float = 0.000328749071416573

    # 冬眠解除における気温の影響度 (k_temp)
    wakeup_k_temp: float = 0.03539096134917101

    # 冬眠開始における気温の影響度 (k_enter_temp)
    hibernation_k_temp: float = 0.16531992109814683

    # 飢餓による探索半径の拡大係数 (hunger_radius_k)
    hunger_radius_k: float = 3.821121143297189

    # 人口に対するベース忌避係数 (base_avoidance_k)
    base_avoidance_k: float = 1.717294638391362

    # 人里経験による慣れ（恐怖心減少）のスピード (urban_habituation_k)
    urban_habituation_k: float = 0.1282855758619867

    # 目的地選択のソフトマックス逆温度（合理性の強さ）
    decision_softmax_beta: float = 0.1920588049172352

    # 1日あたりの資源回復割合 (resource_recovery_rate)
    resource_recovery_rate: float = 0.16632791831333366

    # 人為的死亡（捕殺）リスクの増加係数
    kill_pop_k: float = 0.00209954303761729

    # 秋の焦り閾値引き上げ幅 (autumn_panic_offset) 20→30
    autumn_panic_offset: float = 21.038474775984092
...
class Bear:
    ...
    def check_wakeup(self, day, local_temp):
        """冬眠解除判定"""
        ...
        # ※ 基準気温は生態学的な固定値として扱う
        p_wakeup = min(1.0, max(0.0, local_temp - WAKEUP_BASE_TEMP) * k_temp)
    ...
    def check_hibernation(self, day, local_temp, landuse, mast_factor):
        """冬眠開始判定"""
        ...
        p_enter = min(0.9, max(0.0, HIBERNATION_BASE_TEMP - local_temp) * k_enter_temp)
    ...
    def decide_action(self, day, local_temp, local_precip, mother_mode=None):
        """1日の行動モード決定"""
        ...
        # 悪天候（猛暑・大雨）時は休む
        if local_temp >= HOT_TEMP_THRESHOLD or local_precip >= HEAVY_RAIN_THRESHOLD:
    ...
    def decide_destination(
        ...
        # # ==================================================
        # # ★追加：集団全体のスコア傾向をCSVに記録（サンプリングロギング）
        # # ==================================================
        # # 全員記録すると重いため、IDの下1桁が0のクマ（全体の10%）だけ記録する
        # if self.id % 10 == 0:
        #     import os, csv
        #     log_file = "score_trends_log.csv"
        #     # ファイルが存在しない場合はヘッダーを書き込むためのフラグ
        #     write_header = not os.path.exists(log_file)
            
        #     # finite_maskを適用して、有効な候補地だけの数値を抽出
        #     valid_gains = norm_gains[finite_mask]
        #     valid_avoid = avoidances[finite_mask]
        #     valid_inertia = norm_inertia[finite_mask]
            
        #     with open(log_file, "a", newline="") as f:
        #         writer = csv.writer(f)
        #         if write_header:
        #             writer.writerow(["day", "bear_id", "candidates_count", 
        #                              "gain_mean", "gain_max", 
        #                              "avoid_mean", "avoid_max", 
        #                              "inertia_mean", "total_mean", "hunger_factor", "experience_factor"])
                
        #         writer.writerow([
        #             day, 
        #             self.id,
        #             len(candidates),
        #             f"{valid_gains.mean():.4f}", f"{valid_gains.max():.4f}",
        #             f"{valid_avoid.mean():.4f}", f"{valid_avoid.max():.4f}",
        #             f"{valid_inertia.mean():.4f}",
        #             f"{total_scores.mean():.4f}",
        #             f"{hunger_factor:.4f}", f"{experience_factor:.4f}"
        #         ])
...
class World:
    ...
    def initialize_all_bears(self, total_n):
        """シミュレーション開始時の全個体の生成と初期配置"""
        suitability_map = np.load(HABITAT_SUITABILITY_NPY)
        landuse = self.env[0]
        
        # 「スポーン許可マス（森林等）」かつ「適性度が0より大きい」マスを抽出
        valid_mask = (landuse == SPAWN_LANDUSE_CODE) & (suitability_map > 0)
        valid_y, valid_x = np.where(valid_mask)
        num_valid = len(valid_y)
        
        if num_valid == 0:
            raise ValueError("No valid spawn cells found based on landuse and habitat suitability.")

        # 該当マスの適性度スコアを取得し、確率（合計1.0）に変換
        suitability_scores = suitability_map[valid_y, valid_x]
        probabilities = suitability_scores / np.sum(suitability_scores)

        # ★適性度の重みに基づいて、全個体(total_n)分の配置先インデックスを一括で選ぶ
        chosen_indices = np.random.choice(num_valid, size=total_n, p=probabilities, replace=True)
        ...
        for age, gender, idx in zip(assigned_ages, assigned_genders, chosen_indices):
    ...
    def move_all_bears(self, day, daily_weather, pref_map):
        """強者順に、行動→移動→更新→死亡判定→依存個体同期を行う1日のメインループ"""
        
        # 全体処理（資源回復など）で self.current_temp が使われる時のための「代表値」
        # （迷ったらとりあえず秋田県(ID: 3)の気候をセットしておく安全装置）
        default_pref = 3 if 3 in daily_weather else list(daily_weather.keys())[0]
        self.current_temp = float(daily_weather[default_pref][0])
        self.current_precip = float(daily_weather[default_pref][1])
        ...
        for bear in sorted_bears:
            # =========================================================
            # ★新規：足元の県コードを取得し、このクマ専用の「局所気象」を決定！
            # =========================================================
            pref_id = int(pref_map[bear.y, bear.x])
            
            # もし海や県外(0など)、あるいはデータがないマスの場合は代表県（秋田）の天気を使う
            if pref_id not in daily_weather:
                pref_id = default_pref
                
            local_temp = float(daily_weather[pref_id][0])
            local_precip = float(daily_weather[pref_id][1])
            # =========================================================
            ...
            if bear.mode == HIBERNATION:
                woke = bear.check_wakeup(day, local_temp)
                if not woke:
                    bear.update_energy_hibernation()
                action = bear.mode
                dx, dy = 0, 0
            else:
                bear.decide_action(day, local_temp, local_precip)
                ...
                # ★修正：冬眠開始判定にもその県の気温(local_temp)を渡す
                bear.check_hibernation(day, local_temp, lu, current_mast_factor)
...
# --- 入出力パス ---
VEG_CSV = r"G:\マイドライブ\クマ出没予測\試作\ABM_2\vegetation_gain_table_tohoku.csv"
WEATHER_CSV = r"G:\マイドライブ\クマ出没予測\試作\ABM_2\wether_akita_2023.csv"
ENV_NPY = r"G:\マイドライブ\クマ出没予測\試作\ABM_2\環境データ\env_data_tohoku.npy"
...
def flush_buffer_to_csv(buffer_rows, output_csv, first_write):
    ...
    df.to_csv(output_csv, mode="a", header=first_write, index=False, encoding="utf-8-sig", chunksize=50000)
    buffer_rows.clear()
    return False

def run_single_simulation(ensemble_id: int, config: SimConfig = None, mast_level: str = "ExtremelyPoor", initial_pop_size: int = 18507):
    ...
    # ★追加：都道府県マップの読み込み
    pref_map = np.load(PREF_MAP_NPY) 

    # weather_df = pd.read_csv(WEATHER_CSV)
    # weather_table = weather_df[["temp", "precip"]].values

    weather_tables = {}
    for pref_id, csv_path in WEATHER_CSVS.items():
        df = pd.read_csv(csv_path)
        weather_tables[pref_id] = df[["temp", "precip"]].values
    ...
    output_csv = fr"C:\Users\sueda\ABM_tohoku\result_ensemble_tohoku_{ensemble_id}_{initial_pop_size}_{mast_level}.csv"
    ...
    # ========================================================
    # ★追加：都道府県ごとの集計リスト（辞書型で管理）
    # ========================================================
    pref_names = {1: "青森", 2: "岩手", 3: "宮城", 4: "秋田", 5: "山形"}
    valid_prefs = list(pref_names.keys())

    pref_urban_appearances = {p: [0] * 12 for p in valid_prefs}
    pref_urban_lactating = {p: [0] * 12 for p in valid_prefs}
    pref_human_kills = {p: [0] * 12 for p in valid_prefs}
    pref_energy_sum = {p: [0.0] * 12 for p in valid_prefs}
    pref_days_count = {p: [0] * 12 for p in valid_prefs}
    pref_killed_ages = {p: [0] * 21 for p in valid_prefs}
    pref_natural_deaths = {p: 0 for p in valid_prefs}

    known_dead_bears = set()

    # --- 開始時点の総個体数と内訳を取得（全体 ＆ 県別） ---
    initial_cu_count = 0
    initial_non_cu_count = 0
    pref_initial_cu = {p: 0 for p in valid_prefs}
    pref_initial_non_cu = {p: 0 for p in valid_prefs}
    ...
    for b in world.bears:
        ...
        if is_cu:
            ...
            initial_non_cu_count += 1
            
        p_id = int(pref_map[b.y, b.x])
        if p_id in valid_prefs:
            if is_cu:
                pref_initial_cu[p_id] += 1
            else:
                pref_initial_non_cu[p_id] += 1

    initial_total_bears = initial_cu_count + initial_non_cu_count
    pref_initial_total = {p: pref_initial_cu[p] + pref_initial_non_cu[p] for p in valid_prefs}
    ...
    for day in range(START_DAY, END_DAY + 1):
        ...
        #　その日の「全県の気象データ」を辞書にまとめる
        daily_weather = {
            pref_id: (float(table[day - 1, 0]), float(table[day - 1, 1]))
            for pref_id, table in weather_tables.items()
        }

        # メイン処理（Worldに daily_weather と pref_map を渡す）
        world.move_all_bears(day, daily_weather, pref_map)
        ...
        # 県別のその日の生存数とエネルギー合計
        pref_alive_count = {p: 0 for p in valid_prefs}
        pref_daily_energy = {p: 0.0 for p in valid_prefs}

        for b in world.bears:
            buffer_rows.append({
                "day": day, "bear_id": b.id, "x": b.x, "y": b.y,
                "mode": b.mode, "energy": b.energy, "age_class": b.age_class,
                "age": b.age, "gender": b.gender, "repro_status": b.repro_status,
                "cub_count": b.cub_count, "is_independent": b.is_independent,
                "is_alive": b.is_alive, "death_cause": b.death_cause
            })

            # クマの現在地の都道府県IDを取得（海や県外の場合は0などになる想定）
            p_id = int(pref_map[b.y, b.x])
            ...
            # --- 死亡個体の集計 ---
                ...
                if b.id not in known_dead_bears:
                    ...
                    if b.death_cause == "HUMAN":
                        ...
                        if p_id in valid_prefs:
                            pref_human_kills[p_id][m_idx] += 1
                            pref_killed_ages[p_id][age_idx] += 1
                            
                    elif b.death_cause in ["ENERGY", "ORPHANED", "OLD_AGE"]:
                        annual_natural_deaths += 1
                        if p_id in valid_prefs:
                            pref_natural_deaths[p_id] += 1

        # その日の「集団の平均エネルギー」を加算（全体）
        if alive_bears_count > 0:
            monthly_energy_sum[m_idx] += (daily_energy_total / alive_bears_count)
        monthly_days_count[m_idx] += 1

        # その日の「集団の平均エネルギー」を加算（県別）
        for p in valid_prefs:
            if pref_alive_count[p] > 0:
                pref_energy_sum[p][m_idx] += (pref_daily_energy[p] / pref_alive_count[p])
            pref_days_count[p][m_idx] += 1

        buffer_day_counter += 1

        if buffer_day_counter >= OUTPUT_BUFFER_DAYS:
    ...
    # 1. 全体の指標計算（既存通り）
    total_kills = sum(monthly_human_kills)
    metrics = {
        "sighting_trend": monthly_urban_appearances,
        "family_ratio": [(monthly_urban_lactating[i] / monthly_urban_appearances[i]) if monthly_urban_appearances[i] > 0 else 0.0 for i in range(12)],
        "kill_trend": monthly_human_kills,
        "annual_kill_rate": (total_kills / initial_total_bears) if initial_total_bears > 0 else 0.0,
        "intrinsic_growth_rate": (initial_cu_count - annual_natural_deaths) / initial_non_cu_count + 1.0 if initial_non_cu_count > 0 else 1.0,
        "energy_trend": [(monthly_energy_sum[i] / monthly_days_count[i]) if monthly_days_count[i] > 0 else 0.0 for i in range(12)],
        "kill_age_distribution": [count / total_kills for count in killed_ages_count] if total_kills > 0 else [0.0] * 21
    }

    # 2. 県別の指標計算とmetricsへの追加
    for p in valid_prefs:
        p_name = pref_names[p]
        p_total_kills = sum(pref_human_kills[p])
        p_initial_total = pref_initial_total[p]
        p_initial_non_cu = pref_initial_non_cu[p]
        p_initial_cu = pref_initial_cu[p]
        p_natural_deaths = pref_natural_deaths[p]

        metrics[f"{p_name}_sighting_trend"] = pref_urban_appearances[p]
        metrics[f"{p_name}_family_ratio"] = [(pref_urban_lactating[p][i] / pref_urban_appearances[p][i]) if pref_urban_appearances[p][i] > 0 else 0.0 for i in range(12)]
        metrics[f"{p_name}_kill_trend"] = pref_human_kills[p]
        metrics[f"{p_name}_annual_kill_rate"] = (p_total_kills / p_initial_total) if p_initial_total > 0 else 0.0
        metrics[f"{p_name}_intrinsic_growth_rate"] = (p_initial_cu - p_natural_deaths) / p_initial_non_cu + 1.0 if p_initial_non_cu > 0 else 1.0
        metrics[f"{p_name}_energy_trend"] = [(pref_energy_sum[p][i] / pref_days_count[p][i]) if pref_days_count[p][i] > 0 else 0.0 for i in range(12)]
        metrics[f"{p_name}_kill_age_distribution"] = [count / p_total_kills for count in pref_killed_ages[p]] if p_total_kills > 0 else [0.0] * 21

    # ========================================================
    # ★ 評価指標CSV出力（全体 ＋ 全6県分が1つの表になります）
    # ========================================================
    metrics_csv_path = fr"G:\マイドライブ\クマ出没予測\試作\ABM_2\result\metrics_ensemble_tohoku_{ensemble_id}_{initial_pop_size}_{mast_level}.csv"
    
    metrics_df = pd.DataFrame(dict([(k, pd.Series(v)) for k, v in metrics.items()]))
    metrics_df.to_csv(metrics_csv_path, index=False, encoding='utf-8-sig')
    print(f"📊 評価指標(全体+県別)を {metrics_csv_path} に保存しました！")
    # ========================================================
