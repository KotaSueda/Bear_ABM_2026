...
class SimConfig:
    ...
    # 授乳中の母親にかかる消費エネルギー倍率（親子連れの出没リスクを左右する）
    lactating_cost_mult: float = 1.2
...
class Bear:
    ...
    def update_energy_hibernation(self):
        """冬眠中の低代謝消費"""
        # Configからパラメータを読み込む
        alpha = self.config.energy_cost_alpha
    ...
    def update_energy(self, action, dx, dy, world, day):
        ...
        if self.repro_status == "LACTATING":
            cost *= self.config.lactating_cost_mult
    ...
    def decide_destination(
        ...
        curve_value = 2.0 * (1.0 - (1.0 - e_ratio)**HUNGER_FEAR_RATIO)
...
def run_single_simulation(ensemble_id: int, config: SimConfig = None, mast_level: str = "Great", initial_pop_size: int = 1504):
    ...
    # ========================================================
    # ★ 評価指標7項目のCSV出力（出力しない時は下の数行をコメントアウト）
    # ========================================================
    metrics_csv_path = f"metrics_ensemble_{ensemble_id}_{initial_pop_size}_{mast_level}.csv"
    
    # 12ヶ月分、21年齢分、単一の数値など、長さがバラバラのデータを
    # 自動で縦に揃えて表（DataFrame）に変換するPandasの魔法
    metrics_df = pd.DataFrame(dict([(k, pd.Series(v)) for k, v in metrics.items()]))
    
    # CSVとして保存（BOM付きUTF-8で保存し、Excelでの文字化けを防ぐ）
    metrics_df.to_csv(metrics_csv_path, index=False, encoding='utf-8-sig')
    print(f"📊 評価指標7項目を {metrics_csv_path} に保存しました！")
    # ========================================================
